...
def visualization(dataframes):
    ...
    for i in range(0, 6):
        ...
        max_row = start_row  # 학년별 최대 행을 추적
        ...
        if max_row > start_row:
            sheet.merge_cells(start_row=start_row, start_column=1, end_row=max_row, end_column=1)
        ...
        # 경계선 설정
        for col in range(2, sheet.max_column): # 밑줄
            sheet.cell(row=max_row, column=col).border = Border(
                bottom=Side(style="thin")
            )
        for col in range(1, sheet.max_column + 1, 13): # 날짜줄
            sheet.cell(row=max_row, column=col).border = Border(
    ...
    wb.save(f"./test/{current_datetime}_schedule.xlsx")
    time.sleep(1)
...
currentschedule = backtracking.currentschedule
...
for i in range(option.result_number):  # 10번 반복
    ...
    originalLectures = sorted(originalLectures, key=lambda lecture: not (lecture.lectureCode, lecture.division, lecture.TP))
    originalProfessors = sorted(originalProfessors, key=lambda professor: not professor.isprof)
    originalClassrooms = sorted(originalClassrooms, key=lambda classroom: (classroom.building, classroom.classroomNo))

    # 시간표 생성 및 저장
    dataframes = []  # 모든 시간표를 저장할 공간
    titles = []
    visualization(dataframes)
    print(f"{i+1}번째 시간표 제작 완료")
    time.sleep(1) # 파일 수정 공백
    
    if i < option.result_number - 1:
        # 각 반복마다 모듈을 다시 로드하여 초기화
